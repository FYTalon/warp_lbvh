...
def intersects(lhs: aabb, rhs: aabb) -> bool:
    ...
    lhs_upper = lhs.get_row(0)
    lhs_lower = lhs.get_row(1)

    rhs_upper = rhs.get_row(0)
    rhs_lower = rhs.get_row(1)
...
def merge(lhs: aabb, rhs: aabb) -> aabb:
    ...
    lhs_upper = lhs.get_row(0)  # vec3 for lhs upper bound
    lhs_lower = lhs.get_row(1)  # vec3 for lhs lower bound
    rhs_upper = rhs.get_row(0)  # vec3 for rhs upper bound
    rhs_lower = rhs.get_row(1)  # vec3 for rhs lower bound
    ...
    ret.set_row(0, merged_lower)
    ret.set_row(1, merged_upper)
